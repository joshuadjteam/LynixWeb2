                ...
                <div className="flex-grow overflow-y-auto">
                    {users.map(user => (
                        <button
                            ...
                            className={`w-full text-left p-4 flex items-center gap-3 transition ${selectedUser?.id === user.id ? 'bg-purple-600/50' : 'hover:bg-gray-700/50'}`}
                        >
                            ...
                            {messages.map((msg) => (
                                <div key={msg.id} className={`flex items-start gap-3 ${msg.sender_id === currentUser.id ? 'justify-end' : 'justify-start'}`}>
                                    {msg.sender_id !== currentUser.id && (
                                        <div className="w-8 h-8 bg-gray-600 rounded-full flex-shrink-0 flex items-center justify-center font-bold">
                                            {selectedUser.username.charAt(0).toUpperCase()}
                                        </div>
                                    )}
                                    <div className={`max-w-md p-3 rounded-2xl ${msg.sender_id === currentUser.id ? 'bg-blue-600 rounded-br-none' : 'bg-gray-700 rounded-bl-none'}`}>
                        ...
                        <footer className="p-4 border-t border-gray-700">
                            <div className="flex items-center bg-gray-800 rounded-xl p-2">
                                <textarea
                                    ...
                                    className="flex-1 bg-transparent p-2 text-white resize-none focus:outline-none placeholder-gray-400"
                                    rows={1}
                                    disabled={isLoading}
                                ></textarea>
                                <button onClick={handleSendMessage} disabled={!newMessage.trim() || isLoading} className="p-3 text-white disabled:text-gray-500 hover:text-blue-400 transition rounded-full bg-gray-700 disabled:bg-gray-800">
                                    <SendIcon />
                                </button>
                            </div>
                        </footer>
